...
def InitGraph(G,mu1,mu2,mu3,mu4,mu5,lam1,lam2,EventQ):
  for node in G.__iter__():
    if G.nodes[node]['state'] == 'I':
      FROM_I_TO_Event(node,mu1,mu2,0,EventQ)
    if G.nodes[node]['state'] == 'J':
      J_TO_RJ_Event(node,mu3,0,EventQ)
...
def J_TO_RJ_Event(node,mu3,tGlobal,EventQ):
  tEvent = tGlobal + np.random.exponential(mu3)
  e = Event(state = 'J_TO_RJ',time = tEvent,srcNode = node, targetNode = None)
...
tGlobal = 0
start =time.perf_counter()
InitGraph(G,mu1,mu2,mu3,mu4,mu5,lam1,lam2,EventQ)
...
while True:
  ...
  if e.state == 'I_TO_J':
    ...
    J_TO_RJ_Event(e.srcNode,mu3,tGlobal,EventQ)
    ...
    if G.nodes[e.targetNode]['state'] == 'S':
      ...
      FROM_I_TO_Event(e.targetNode,mu1,mu2,tGlobal,EventQ)
    ...
    if G.nodes[e.targetNode]['state'] == 'S' or G.nodes[e.targetNode]['state'] == 'RI':
      ...
      J_TO_RJ_Event(e.targetNode,mu3,tGlobal,EventQ)
      GET_J_Event(e.srcNode,lam2,tGlobal,EventQ)
